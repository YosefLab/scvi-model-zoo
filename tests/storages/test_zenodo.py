...
_TEST_ZENODO_RECORD = "966897"


def test_zenodo_storage_init(save_path):
    store = ZenodoStorage(_TEST_ZENODO_RECORD, save_path, sandbox=True)
    assert store._record_id == _TEST_ZENODO_RECORD
    assert store._data_dir == save_path
    assert (
        store._zenodo_record_url_template
        == "https://sandbox.zenodo.org/record/{}/files/{}"
    )
    assert store._zenodo_api_base_url == "https://sandbox.zenodo.org/api/"
    assert store._zenodo_api_records_url == "https://sandbox.zenodo.org/api/records/"
