...
def test_reference(save_path):
    ...
    assert models_df.index[0] == "80262d08-4a30-4071-a3c6-96274182646d.zip"
    ...
    assert models_df["class_name"].iloc[0] == "scvi.model.SCVI"

    datasets_df = generic_ref.get_datasets_df()
    assert len(datasets_df) == 1
    assert datasets_df.index[0] == "dcfaad7a-70a4-4669-87d2-7bb241673097.h5ad"
...
def test_reference_load_model(save_path):
    ...
    model = generic_ref.load_model("80262d08-4a30-4071-a3c6-96274182646d.zip")
...
def test_reference_save_dataset(save_path):
    ...
    dsm = DatasetMetadata(
        tissue="Misc", is_cite=True, has_latent_embedding=True, is_annotated=False
    )
    dataset_id = generic_ref.save_dataset(dummyfile_path, None, True, dsm)

    datasets_df = generic_ref.get_datasets_df()
    assert len(datasets_df) == 2
    existing_dataset_id = "dcfaad7a-70a4-4669-87d2-7bb241673097.h5ad"
    assert datasets_df.columns.to_list() == [
        "cell_count",
        "gene_count",
        "tissue",
    ...
    assert bool(datasets_df["has_cite"].loc[existing_dataset_id]) is False
    assert bool(datasets_df["has_cite"].loc[dataset_id]) is True
...
def test_reference_save_model(save_path):
    ...
    mm = ModelMetadata(
        cls_name="scvi.model.TOTALVI",
        train_dataset="dcfaad7a-70a4-4669-87d2-7bb241673097.h5ad",
        n_hidden=128,
        n_layers=1,
        n_latent=10,
    ...
    existing_model_id = "80262d08-4a30-4071-a3c6-96274182646d.zip"
    assert models_df.columns.to_list() == [
        "class_name",
        "train_dataset",
        "n_hidden",
    ...
    assert models_df["class_name"].loc[existing_model_id] == "scvi.model.SCVI"
    assert models_df["class_name"].loc[model_id] == "scvi.model.TOTALVI"

    assert (
        models_df["train_dataset"].loc[existing_model_id]
        == "dcfaad7a-70a4-4669-87d2-7bb241673097.h5ad"
    )
    assert (
